...
- **Placeholder scan:** The bullet now points readers at the wave-orchestration recipe for worktree creation and keeps only the inline cleanup example `git worktree remove "<path>"`. The `"<path>"` is a shell-context placeholder (quoted to be copy-paste-safe, literal angle brackets inside quotes, reader substitutes their own path) — not a plan failure. Task 2 Step 4's `<the-number-gh-pr-create-just-printed>` is a runtime substitution (implementer reads the PR number from Step 3's output). Task 2 Step 6's `<N>` and `<sha>` are the same pattern.
